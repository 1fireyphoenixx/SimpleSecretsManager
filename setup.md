...
unchanged. An out-of-scope path returns 403, a missing/deleted secret returns 404 unless **Allow creation** is enabled,
...
Upgrading from 0.0.1 or 0.0.2 preserves existing secrets and agents. Existing
write credentials remain update-only until an administrator enables creation.
This optional permission uses the existing JSON record storage; no SQL migration
is needed. Creation starts a new secret at revision 1; recreating a deleted secret
continues its previous revision sequence. Concurrent requests are serialized in
the same transaction as the existence check. All successful writes return 200.
...
| POST | `/write/<path>` | Dedicated write bearer; update or, with `allow_create`, create using `{value}` |
