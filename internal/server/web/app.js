...
function signedOut(){csrf='';$('dashboard').hidden=true;$('login').hidden=false;$('logout').hidden=true;$('status').textContent='Authentication required';$('new-token').textContent='';$('revealed-value').textContent='';$('reveal').close();}
...
async function refresh(){await status();const [secrets,agents,enrollments,admins]=await Promise.all(['secrets','agents','enrollment','administrators'].map(p=>api('admin/'+p)));
